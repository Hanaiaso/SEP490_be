...
namespace VietTien.API.Services.Interfaces
{
    ...
    {
        Task<PaginatedList<InventoryItemDto>> GetInventoryByWarehouseAsync(Guid warehouseId, string? search, int? minQty, int? maxQty, DateTime? fromDate, DateTime? toDate, string? itemType, int pageNumber, int pageSize);
        ...
        Task<List<ExcessStockAlertDto>> GetExcessStockAlertsAsync();
    }
}
