...
namespace VietTien.API.Services.Interfaces
{
    public interface IOrderService
    {
        ...
        Task<SePayQrResponseDto> GenerateSePayQrAsync(Guid orderId);
        Task ProcessSePayWebhookAsync(SePayWebhookDto payload, string providedToken);
        Task<PaymentStatusResponseDto> GetPaymentStatusAsync(Guid orderId);
        Task<SalesDashboardStatsDto> GetSalesDashboardStatsAsync();
        Task ConfirmDirectOrderPaymentAsync(Guid orderId);
        Task UploadInvoicePdfAsync(Guid orderId, string pdfBase64);
        ...
        Task<SalesOrderDetailDto> GetSalesOrderDetailAsync(Guid orderId);
